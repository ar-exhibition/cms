...
CREATE TABLE IF NOT EXISTS `cmsDatabase`.`Studies` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `Studies_UNIQUE` (`Studies` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cmsDatabase`.`Creator` (
  ...
  CONSTRAINT `fk_Creator_Studies`
    FOREIGN KEY (`Studies`)
    REFERENCES `cmsDatabase`.`Studies` (`Studies`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cmsDatabase`.`Course` (
  ...
  PRIMARY KEY (`CourseID`, `Studies`),
  UNIQUE INDEX `CourseID_UNIQUE` (`CourseID` ASC) VISIBLE,
  INDEX `fk_Course_Studies1_idx` (`Studies` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `cmsDatabase`.`Scene` (
  ...
  `Name` VARCHAR(45) NULL,
  `SceneFile` VARCHAR(1024) NULL,
  `MarkerFile` VARCHAR(1024) NULL,
  PRIMARY KEY (`SceneID`),
